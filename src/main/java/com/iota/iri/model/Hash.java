...
public final class Hash implements Serializable, Indexable {
    ...
    // INNER CLASS FIELDS CANNOT BE ACCESSED UNTIL ENTIRE CLASS IS INSTANTIATED
    // WHEN USED WITH THE SEMANTICS IN THIS CLASS BELOW IS THREADSAFE
    ...
    private final class TritSafe {
        ...
        private TritSafe(int[] trits) {
            this.trits = Objects.requireNonNull(trits);
        }
    }
    ...
    public Hash(final byte[] source, final int sourceOffset, final int sourceSize) {
        ...
        // TODO - THIS IS A BUGGY BOUNDS-SIZE CHECK. I'LL KEEP IT IN BECAUSE TOO MANY TESTS FAIL OTHERWISE.
    ...
    public int[] trits() {
        ...
        if (safe == null) {
            synchronized (lock) {
                if (tritSafe == null) {
                    Objects.requireNonNull(byteSafe, "I need my bytes to be set in the constructor " +
                            "or by persistence in order to construct trits");
                    byte[] src = bytes();
    ...
    public byte[] bytes() {
        ...
        if (safe == null) {
            synchronized (lock) {
                if (byteSafe == null) {
                    Objects.requireNonNull(tritSafe, "I need my trits to be set in the constructor " +
                            "in order to construct trits");
                    int[] src = trits();
                    byte[] dest = new byte[SIZE_IN_BYTES];
